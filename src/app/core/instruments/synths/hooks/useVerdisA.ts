...
export default function useVerdisA() {
  type TScale = { [key: string]: { freq: number; waveLength: number } };
  ...
  function getFrequency(key: keyof TScale) {
    return scale[key].freq as Frequency;
  }

  function getWaveLength(key: keyof TScale) {
    return scale[key].waveLength;
  }
